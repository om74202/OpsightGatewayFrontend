...
const METRIC_INFO_TEXT =
  "The ipv4.router-metric defines route priority; lower numbers are preferred when multiple paths exist.";
...
export const StaticIPConfiguration = () => {
  ...
  const openModal = (type, profile = null) => {
    ...
    if (profile) {
      reset({
        ...
        routerMetric: profile.routerMetric ?? "",
      });
    } else {
      reset({
        name: "",
  ...
  const onSubmit = async (values) => {
    try {
      const metricInput = values.routerMetric;
      const routerMetricValue =
        metricInput === undefined || metricInput === null || metricInput === ""
          ? null
          : Number(metricInput);
      ...
      const payload = {
        type: modalType,
        ...values,
        metric: routerMetricValue,
      };

      if (editMode) {
        await axios.put(`${process.env.REACT_APP_API_URL}/gatewayConfig/IPConfig`, payload);
        notify.success("Configuration edited successfully");
      } else {
        await axios.post(`${process.env.REACT_APP_API_URL}/gatewayConfig/IPConfig`, payload);
          ...
          <tbody>
            {profiles.map((p, i) => (
              <tr key={i} className="hover:bg-gray-50">
                ...
                <td className="py-2 px-4 border-b">{p.routerMetric}</td>
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              <div className="mb-6">
                ...
                <input
                  type="number"
                  className={`w-full px-4 py-2 border rounded-md mt-2 ${
                    errors.routerMetric ? "border-red-400 bg-red-50" : "border-gray-300"
                  }`}
                  placeholder="e.g. 10"
                  {...register("routerMetric", {
                    min: { value: 0, message: "Metric must be 0 or greater" },
                  })}
                />
                {errors.routerMetric && (
                  <p className="text-xs text-red-600 mt-1">{errors.routerMetric.message}</p>
                )}
              </div>
